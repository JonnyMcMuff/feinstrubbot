...
def step_impl(context):
    ...
    airQuality.message.text = "My current location is xxxx"
    context.feinstaub.text(bot, airQuality)

@then('the data is updated in the backend')
def step_impl(context):
    context.feinstaub.users.update.assert_called_once()
    return context.feinstaub.bot.sendMessage.assert_called_with(chat_id=1337,
                                                                text=AnyStringWith("Thank you for your location update"))
